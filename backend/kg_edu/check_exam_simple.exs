...
# Check student exam
IO.puts "\n=== Student Exams ==="
case Ash.read(KgEdu.Knowledge.StudentExam, filter: [exam_id: exam_id, student_id: student_id], tenant: tenant) do
  {:ok, student_exams} ->
    ...
    Enum.each(student_exams, fn se ->
      ...
      case Ash.read(KgEdu.Knowledge.StudentExamAnswer, filter: [student_exam_id: se.id], tenant: tenant) do
        {:ok, answers} ->
