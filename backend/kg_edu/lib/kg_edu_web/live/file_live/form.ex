defmodule KgEduWeb.FileLive.Form do
            ...
            <div class="mt-2">
              <%= if String.contains?(@file.file_type, "image") do %>
                <img
                  src={@file.path}
                  alt={@file.filename}
                  class="max-w-full h-auto rounded-md shadow-sm max-h-96 object-contain"
                />
              <% else %>
  ...
  def mount(params, _session, socket) do
    ...
    {:ok,
     ...
     |> allow_upload(:file,
       accept: ~w(.jpg .jpeg .png .gif .pdf .doc .docx .txt .zip .rar .mp4 .mov .avi .mp3 .wav),
       max_entries: 1
     )
     |> assign_form()}
  end
      ...
      consume_uploaded_entries(socket, :file, fn %{path: path}, entry ->
        ...
        upload_params =
          Map.put(
            upload_params,
            "course_id",
            file_params["course_id"] || socket.assigns.course_id
          )
