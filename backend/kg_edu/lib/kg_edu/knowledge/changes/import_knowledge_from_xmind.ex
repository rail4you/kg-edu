defmodule KgEdu.Knowledge.Changes.ImportKnowledgeFromXmind do
  ...
  def change(changeset, opts, context) do
    Ash.Changeset.after_action(changeset, fn changeset, record ->
      ...
      course_id =
        Ash.Changeset.get_argument(changeset, :course_id) || opts[:course_id] || record.course_id
          ...
          {:ok, xmind_parsed_data} ->
            case KgEdu.XmindParser.convert_to_knowledge_resources(xmind_parsed_data, course_id) do
              {:ok, knowledge_resources} ->
                ...
                results =
                  Enum.map(knowledge_resources, fn resource_attrs ->
                ...
                case Enum.find(results, fn result ->
                  ...
                  nil ->
                    Logger.info(
                      "Successfully imported #{length(knowledge_resources)} knowledge resources from XMind"
                    )
                  ...
                  {:error, error} ->
                    Logger.error(
                      "Failed to import some knowledge resources from XMind: #{inspect(error)}"
                    )
              ...
              {:error, reason} ->
                Logger.error(
                  "Failed to convert XMind data to knowledge resources: #{inspect(reason)}"
                )
  ...
  def import_knowledge_from_xmind(base64_data, course_id, opts \\ []) do
    ...
    changeset =
      Ash.Changeset.for_create(%KgEdu.Knowledge.Resource{}, :create, %{
