defmodule KgEdu.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%KgEdu.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(KgEdu.Accounts.User, id, authorize?: false) do
