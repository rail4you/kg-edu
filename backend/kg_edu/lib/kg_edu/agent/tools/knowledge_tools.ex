defmodule KgEdu.Agent.Tools.GetKnowledgeResources do
  ...
  def run(params, _context) do
    ...
    resources = KgEdu.Agent.DataAccess.list_knowledge_resources(tenant, params[:courseId])
...
defmodule KgEdu.Agent.Tools.GetExercises do
  ...
  @impl true
  def run(params, _context) do
    tenant = params[:_tenant] || nil
    exercises = KgEdu.Agent.DataAccess.list_exercises(tenant, params[:courseId], params[:knowledgeResourceId])

    text =
      "共 #{length(exercises)} 道练习题:\n" <>
        (exercises
         |> Enum.map(fn e ->
...
defmodule KgEdu.Agent.Tools.GetExams do
  ...
  def run(params, _context) do
    tenant = params[:_tenant] || nil
    exams = KgEdu.Agent.DataAccess.list_exams(tenant, params[:courseId])
