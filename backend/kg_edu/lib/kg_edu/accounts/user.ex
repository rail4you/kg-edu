defmodule KgEdu.Accounts.User do
  ...
  actions do
    ...
    create :create_user do
      ...
      argument :edit_enabled, :boolean do
        description "The user's edit permission switch (编辑权限开关)"
        allow_nil? true
        default true
      end

      argument :edit_period_start, :date do
        description "The user's edit period start date (编辑起始日期)"
        allow_nil? true
      end

      argument :edit_period_end, :date do
        description "The user's edit period end date (编辑截止日期)"
        allow_nil? true
      end
    ...
    update :update do
      ...
      validate fn changeset, _context ->
        start_date = Ash.Changeset.get_attribute(changeset, :edit_period_start)
        end_date = Ash.Changeset.get_attribute(changeset, :edit_period_end)

        if start_date && end_date && Date.compare(end_date, start_date) == :lt do
          {:error, field: :edit_period_end, message: "截止日期不能早于起始日期"}
        else
          :ok
        end
      end
    ...
    action :import_users_from_excel, :map do
      ...
      argument :attributes, {:array, :atom} do
        description "List of attributes in order: [:name, :phone, :email, :password, :colledge, :major, :class]"
  ...
  attributes do
    ...
    attribute :edit_enabled, :boolean do
      ...
      default true
      public? true
      description "教师端编辑权限开关 (Teacher edit permission switch)"
    end

    attribute :edit_period_start, :date do
      allow_nil? true
      public? true
      description "教师可编辑起始日期 (Teacher edit period start date)"
    end

    attribute :edit_period_end, :date do
      allow_nil? true
      public? true
      description "教师可编辑截止日期 (Teacher edit period end date)"
    end
  end
