...
case Ash.get(KgEdu.Knowledge.Exam, exam_id, tenant: tenant) do
  {:ok, exam} ->
    ...
    # Get exam exercises
    IO.puts "\n=== Exam Exercises ==="
    exercises_query = Ash.Query.filter(KgEdu.Knowledge.ExamExercise, exam_id == ^exam_id)
...
# Check student exam
IO.puts "\n=== Student Exams ==="
student_exam_query = Ash.Query.filter(KgEdu.Knowledge.StudentExam, exam_id == ^exam_id and student_id == ^student_id)
...
case Ash.read(student_exam_query, tenant: tenant) do
  {:ok, student_exams} ->
    ...
    Enum.each(student_exams, fn se ->
      ...
      answers_query = Ash.Query.filter(KgEdu.Knowledge.StudentExamAnswer, student_exam_id == ^se.id)
